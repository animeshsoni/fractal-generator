...
public class ColorModelFactory {
	...
	/**
	 * Create a color model that contains different shades of red and blue. The
	 * number of colors included in the model is set by the parameter. This
	 * method is created is created by the team whereas the other three models
	 * are created by Prof Hertz.
	 * 
	 * @param numColors 
	 *            Number of different color shades to use.
	 * @return Color model that shows all of the different possible shades of
	...
	public static IndexColorModel createPurpleColorModel(int numColors) {
		...
		for (int i = 0; i < reds.length - 1; i++) {
			reds[i] = (byte) ((Math.log10(i) / Math.log10(reds.length)) * 256);
			greens[i] = (byte) ((Math.log10(i / 4) / Math.log10(reds.length)) * 256);
